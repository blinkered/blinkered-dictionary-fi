/**
 * The collections that attest Finnish, and where each comes from.
 *
 * The largest Gutenberg shelf of any candidate here: 3,681 texts, against a 44,104-word
 * list. Finnish inflects hard, so expect the drop list to be long and mostly inflections.
 *
...
// wearing a Leipzig label, so including one would corroborate `wiki:fi` while looking
// like another family. That is the exact failure the three-families rule exists to catch.
const LEIPZIG = [
  'fin_news_2022_1M',
  'fin_newscrawl_2011_1M',
]

const ALL = [
  {
    id: 'wiki:fi',
    what: 'Finnish Wikipedia — modern encyclopedic prose',
    needs: `${CACHE}fiwiki.xml.bz2`,
    documents: () => wikiDocuments(`${CACHE}fiwiki.xml.bz2`),
  },
  {
    id: 'wikisource:fi',
    what: 'Finnish Wikisource — same Wikimedia family, so it corroborates rather than counts',
    needs: `${CACHE}fiwikisource.xml.bz2`,
    documents: () => wikiDocuments(`${CACHE}fiwikisource.xml.bz2`),
  },
  ...LEIPZIG.map((pkg) => ({
    id: `lz:${pkg}`,
  ...
  {
    id: 'tat',
    from: 'https://downloads.tatoeba.org/exports/per_language/fin/fin_sentences.tsv.bz2',
    what: 'Tatoeba Finnish — contemporary and conversational',
    needs: `${CACHE}fin_sentences.tsv`,
    documents: () => tatoebaDocuments(`${CACHE}fin_sentences.tsv`),
  },
  {
    id: 'fw2',
    from: 'https://huggingface.co/datasets/HuggingFaceFW/fineweb-2/resolve/main/data/fin_Latn/train/000_00000.parquet',
    what: 'FineWeb-2 Finnish — a web crawl nobody here made',
    needs: `${CACHE}fineweb2-fin.parquet`,
    documents: () => fineweb2Documents(`${CACHE}fineweb2-fin.parquet`),
  },
  {
    id: 'gut',
    from: 'https://www.gutenberg.org/cache/epub/feeds/pg_catalog.csv',
    what: 'Project Gutenberg Finnish, 3,681 texts',
    needs: `${CACHE}gutenberg-fi`,
    documents: () => {
      const dir = `${CACHE}gutenberg-fi`
  ...
  {
    id: 'ebible:fin',
    from: 'https://ebible.org/Scriptures/fin_vpl.zip',
    what: 'Finnish Bible — a family nothing else here belongs to',
    needs: `${CACHE}ebible-fin/fin_vpl.txt`,
    documents: () => verseDocuments(`${CACHE}ebible-fin/fin_vpl.txt`),
  },
  {
    id: 'ia',
    ...
    legible: 0.35,
    what: 'Internet Archive Finnish books — literature, and the register a newspaper never reaches',
    needs: `${CACHE}archive-fi`,
    from: 'https://archive.org/search?query=mediatype%3Atexts+AND+language%3A%22Finnish%22',
    documents: () => {
      const dir = `${CACHE}archive-fi`
...
/**
 * Finnish publishers, for the harvest.
 *
 * Chosen because they publish in Finnish rather than because they are large. A harvester
...
export const DOMAINS = [
  'yle.fi', 'hs.fi', 'is.fi', 'iltalehti.fi', 'ts.fi',
  'kaleva.fi', 'savonsanomat.fi', 'aamulehti.fi', 'kauppalehti.fi', 'mtvuutiset.fi',
  'kirjasampo.fi', 'kiiltomato.net',
]
